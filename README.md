...
ReadSON is a [command line utility](#command) with the purpose of converting structred JSON data into a more readable format, such as markdown or plain text. The main use case is to convert large databases, exported ass JSON, into personal knowledge bases, such as [Obsidian](https://obsidian.md/) or [Foam](https://github.com/foambubble/foam).
...
Templates are simple text files composed of *text blocks* and *logic blocks*. Text blocks are kept as is, where logic blocks are either flow control constructs (such as if and for) or data variable accesses, which change depending on the data. Logic blocks are defined by opening and closing `$`.

### Variable Accesses and JSON paths

The simplest logic block is a variable access. This block - the opening and closing of `$` and everything in it - are replaced by the variable they are accessing. So the block `$ age $` is replaced by the age value, for example `28`.

The templates support indexes and JSON paths too. If you have nested objects, you can traverse down with the `->` symbol. So if `name` is as an object with the properties `first` and `last`, you could access the first name by doing `name->first`.
...
The if construct is also suported by templates with the following syntax:
...
Conditions are the gates of if, which determine the clause to evaluate and use in the final result. The boolean operators of `and` and `or` are suportted (using those keywords), and with *ors having priority over ands*. Negation `!` is also possible as well as changing priority/grouping conditions with parenthesis `()`. 
...
- `exists` checks whether a given variable exists in the data, allowing for safe accesses. For example, `$ if exists spouse $ $ spouse->name $ $end$` gets the name of the spose, if there is one. Notice that accessing the name of the spouse is done as a normal access to a variable.
...
For loops are a way to iterate over arrays or object properties. They are essentialy *for each* loops, and not associated with a condition. There are two types of for loops in templates.
...
$ for i, job = range positions$ 
 - $ job->name $ $end$ 
```
- `props` loops iterate over objects and they property/value pairs.
